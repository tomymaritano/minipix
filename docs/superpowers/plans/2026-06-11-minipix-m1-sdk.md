...
**Backlog del review de Task 13**: (a) fixture AVIF 10-bit/grayscale para cubrir las variantes 16-bit del normalizador; (b) [RESUELTO — Path B1] avif-decode 1.0.2 `unprem` tiene una fórmula incorrecta: `(val*256)/(alpha*256)/256` simplifica a `val/alpha/256 ≈ 0` en vez del correcto `val*255/alpha`. avif-parse 2.1.0 expone `AvifData::premultiplied_alpha: bool` (flag HEIF `prem`) y ya lo parseamos en el guard de dimensiones. Se añadió un guard explícito que rechaza AVIFs premultiplicados con `Error::Decode { format: Avif, detail: "premultiplied-alpha … not supported" }` en lugar de dejar que avif-decode corrompa colores silenciosamente. El helper `reject_if_premultiplied()` es testeable sin fixture (covers ambas ramas); el test `avif_parse_exposes_premultiplied_flag` se actualizó para usar una imagen con alpha real, validando que ravif produce straight-alpha correctamente. El TRUE-branch integration test (avif-parse seteando el flag en un container premultiplicado real) requiere aún una fixture AVIF premultiplicada construida externamente — tracked, prioridad baja (ravif no puede generarla; el helper cubre el rechazo tipado). Cuando avif-decode corrija el upstream (o se reemplace), quitar el guard y agregar esa fixture binaria.
